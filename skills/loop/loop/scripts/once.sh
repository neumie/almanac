...
loop_set_run_config \
  ...
  "model=$AGENT_MODEL" \
  "effort=$AGENT_EFFORT"
trap 'loop_finish_run "$?"; rm -f "${LOOP_SNAP_FILE:-}"' EXIT
...
loop_update_run_progress 1 "provider=$PROVIDER iteration=1/1"
...
case "$PROVIDER" in
  claude)
    ...
    # rather than an inline claude exec (#66 — loop migration onto the engine).
    ...
    prompt="@$PROMPT Previous LOOP commits: $loop_commits"
...
$loop_commits"
    ...
    codex_log="docs/plans/${SPEC_NAME}/loop-codex-once.log"
    codex_result=$(mktemp)
    ...
    if [ "${LOOP_CODEX_VERBOSE:-}" = "1" ]; then
      ...
      # rather than an inline codex exec (#66 — loop migration onto the engine).
