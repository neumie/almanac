...
Unlike `ralph-loop` (a slice-queue loop that builds a spec task-by-task), harden-loop is **review-driven**: it discovers and fixes defects in code that already exists. Both sit on the same shared bash engine (the focused `lib/` modules — `lib/run.sh`, `lib/agent.sh`, `lib/worker.sh`, etc.), exposed as `almanac harden <target>`.
...
6. **Commit.** If the target is a git repo, the round's changes (fixer edits, regenerated tests, findings ledger) are committed as `harden(<slug>): round N fixes (C)` with each addressed finding listed in the body as `- [lens] claim` (or a `… round N checkpoint` commit when nothing blocking was fixed). The **loop** writes this message from the kill-list — the fixer is never given git access (it stays in the milder `workspace-write` sandbox). Committing happens **every round, regardless of the feedback verdict** (a still-red round is still a reviewable checkpoint). The `.almanac/` runtime registry is excluded. Start from a **clean tree** (the whole tree minus `.almanac/` is staged). Opt out with `HARDEN_AUTOCOMMIT=0` to leave fixes uncommitted for manual review.
