...
2. For the dimension needing the most attention, run:
   ```bash
   python3 "$UNSLOP_DIR/scripts/calibrate_pairs.py" generate --base passage.txt --dimension DIM --seed N
   ```
...
5. After each round, run:
   ```bash
   python3 "$UNSLOP_DIR/scripts/calibrate_score.py" --preferences preferences.jsonl
   ```
...
6. Use `--next` to pick which dimension to play next:
   ```bash
   python3 "$UNSLOP_DIR/scripts/calibrate_score.py" --preferences preferences.jsonl --next
   ```
...
7. When a sample-derived profile exists (`profile.json`, WP10a), run:
   ```bash
   python3 "$UNSLOP_DIR/scripts/calibrate_score.py" --preferences preferences.jsonl --profile profile.json
   ```
