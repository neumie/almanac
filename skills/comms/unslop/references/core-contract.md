...
```bash
python3 "$UNSLOP_DIR/scripts/validate_preservation.py" original.txt transformed.txt
python3 "$UNSLOP_DIR/scripts/banned_phrase_scan.py" <<< "$OUTPUT"
python3 "$UNSLOP_DIR/scripts/structure_scan.py" <<< "$OUTPUT"
python3 "$UNSLOP_DIR/scripts/silhouette_scan.py" <<< "$OUTPUT"
python3 "$UNSLOP_DIR/scripts/readability_metrics.py" <<< "$OUTPUT"
python3 "$UNSLOP_DIR/scripts/diff_check.py" original.txt transformed.txt
```
...
Remaining structure damage is blocking. Re-diagnose its exact spans, make one
smaller repair, and rerun every gate. Re-read negations, conditions, scope,
certainty, and party relationships before accepting the result.
