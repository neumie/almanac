...
```bash
python3 "$UNSLOP_DIR/scripts/harvest_samples.py" SOURCE [SOURCE...] -o candidates.json
python3 "$UNSLOP_DIR/scripts/harvest_classify.py" --candidates candidates.json --mode heuristic
```
...
Create the project-local voice directory and keep it uncommitted unless the user
explicitly asks to version it:

```bash
VOICE_DIR=".unslop/voice/<name>"
mkdir -p "$VOICE_DIR/samples"
python3 "$UNSLOP_DIR/scripts/voice_profile.py" "$VOICE_DIR/samples" -o "$VOICE_DIR/profile.json"
python3 "$UNSLOP_DIR/scripts/voice_card.py" --profile "$VOICE_DIR/profile.json" --samples "$VOICE_DIR/samples" --out "$VOICE_DIR" --name <name> --provenance
```
...
`references/commands/mimic.md`, run the scanners, then score it:

```bash
python3 "$UNSLOP_DIR/scripts/voice_score.py" \
  --profile "$VOICE_DIR/profile.json" \
  --impostors "$UNSLOP_DIR/assets/voice-impostors" \
  --seed 7 --samples "$VOICE_DIR/samples" demo.md
```

A voiced demo that trips a slop gate fails the loop; do not ship it with an
excuse.
