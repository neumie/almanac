...
`unslop cleanup` or when the user wants suggestions instead of a silent
rewrite. Set `$INPUT` to the source.
...
python3 "$UNSLOP_DIR/scripts/structure_scan.py" <<< "$INPUT"
python3 "$UNSLOP_DIR/scripts/silhouette_scan.py" <<< "$INPUT"
python3 "$UNSLOP_DIR/scripts/readability_metrics.py" <<< "$INPUT"
```
...
```bash
python3 "$UNSLOP_DIR/scripts/suggest.py" document.md            # emit suggestions
python3 "$UNSLOP_DIR/scripts/suggest.py" doc.md --apply-replacements repl.json   # merge model replacements
python3 "$UNSLOP_DIR/scripts/check_suggestions.py" suggestions.json              # blocking contract gates
```
