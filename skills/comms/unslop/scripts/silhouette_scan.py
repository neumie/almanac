...
(assets/silhouette-human-reference.json), negative side clipped. The
...
REFERENCE_PATH = (
    ...
    / "assets" / "silhouette-human-reference.json"
)
...
def load_reference(path: Path) -> dict:
    try:
        ...
        raise ValueError(f"invalid silhouette reference: {exc}") from exc
...
def main(argv: list[str]) -> int:
    ...
    try:
        ...
        print(f"Invalid reference {REFERENCE_PATH}: {exc}", file=sys.stderr)
        return 2
