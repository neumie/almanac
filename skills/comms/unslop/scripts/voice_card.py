...
def _profile_int(value) -> int:
    try:
        return int(value)
    except (TypeError, ValueError) as exc:
        raise ValueError(f"expected numeric profile value, got {value!r}") from exc
...
def _sheet_markers(dim, sentences, profile):
    ...
    lines = [f"- Sentences in samples exercising this: {len(sentences)}.",
             f"- Median length of those sentences: {_profile_int(med)} words."]
    if dim == "numbers-data":
...
def build_card(profile, docs, matrix, name):
    med = _profile_int(profile["sentence_lengths"]["median"])
    iqr = _profile_int(profile["sentence_lengths"]["iqr"])
    ...
    lines.append(
        ...
        f"Paragraphs average {_profile_int(profile['paragraph_stats']['mean_words'])} words."
    )
    if examples:
        lines.append(
...
def main(argv):
    ...
    try:
        ...
        print(f"invalid profile or samples: {exc}", file=sys.stderr)
        return 2
    ...
    try:
        ...
        print(f"could not build voice card: {exc}", file=sys.stderr)
        return 2
    return 0
