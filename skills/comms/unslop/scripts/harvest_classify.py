...
def quality_for(candidate: dict[str, Any], cells: list[str]) -> int:
    words = _as_int(candidate.get("words") or len(re.findall(r"\w+", candidate.get("text", ""))))
    score = 3
    if 40 <= words <= 220:
        score += 1
    if cells:
...
def rank_enriched(candidates: list[dict[str, Any]]) -> list[int]:
    ...
    ranked = sorted(
        ...
        key=lambda row: (
            not row[1],
            -_as_int(candidates[row[0]].get("quality") or 0),
...
def merge_results(candidates_path: Path, results_path: Path) -> dict[str, Any]:
    ...
    for idx, candidate in enumerate(candidates):
        ...
        quality_value = row.get("quality") if row else None
        quality = _as_int(quality_value) if quality_value is not None else quality_for(candidate, cells)
...
def main(argv: list[str]) -> int:
    ...
    try:
        ...
        print(json.dumps({"error": f"could not classify candidates: {exc}"}))
        return 2
    return 0
