...
class StructuralPatternRequired(TypedDict):
    pattern: str
    category: str
    severity: str
    suggestion: str
...
class StructuralPattern(StructuralPatternRequired, total=False):
    min_matches: int
...
BANNED_PHRASES: dict[str, PhraseInfo] = {
...
def scan_for_violations(text: str, include_quoted: bool = False) -> list[Violation]:
    ...
    for pattern_info in STRUCTURAL_PATTERNS:
        ...
        min_matches = pattern_info.get("min_matches", 1)
    ...
    freq_gated = {
        p["category"] for p in STRUCTURAL_PATTERNS if p.get("min_matches", 1) > 1
    }
