...
- **Install-time, per skill (Claude resources):** `~/.claude/skills/almanac/<name>` → `$ALMANAC_HOME/skills/<category>/<name>` (directory symlink). One per skill, flattening the categorized layout.
- **Install-time, shared Agent Skills (Codex + Pi):** `~/.agents/skills/almanac/<name>` → `$ALMANAC_HOME/skills/<category>/<name>`. `almanac install codex` and `almanac install pi` intentionally target the same open-standard location; Codex requires restart, Pi supports `/reload`. `.almanac-install/manifest.tsv` records exact owned targets and `owners/<provider>` keeps one provider's uninstall from removing another's links. Never replace this with substring-based ownership checks.
...
Prefer relative Markdown links for bundled references. For shell commands that need an absolute installed path, document both directory-symlink roots: `~/.agents/skills/almanac/<name>/...` for Codex/Pi and `~/.claude/skills/almanac/<name>/...` for Claude Code. Never use `${CLAUDE_SKILL_DIR}/scripts/...` — the Claude commands path is a single-file symlink and won't resolve subdirectories.
