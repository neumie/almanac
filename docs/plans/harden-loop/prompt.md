...
2. **GitHub issues.** Else if `gh issue list --state open --label "ralph(harden-loop)"` returns at least one issue, that's your queue. Each issue body contains an `## Acceptance criteria` section with `- [ ]` items.
