...
2. **GitHub issues.** Else if `gh issue list --search 'label:"loop(converge-loop)" state:open'` returns at least one issue, that's your queue. (Use `--search`, not `--label` — the parenthesised label name breaks the `--label` filter.) Each issue body contains an `## Acceptance criteria` section with `- [ ]` items.
...
1. Start with `LOOP(converge-loop):` prefix
