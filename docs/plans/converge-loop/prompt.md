...
Pull @docs/plans/converge-loop/prd.md into your context.
...
1. **Local slice files.** If `docs/plans/converge-loop/issues/` contains `*.md` files, that directory is your queue. Each file has frontmatter (`status`, `blocked-by`, `type`) and an `## Acceptance criteria` checklist of `- [ ]` items.
2. **GitHub issues.** Else if `gh issue list --search 'label:"ralph(converge-loop)" state:open'` returns at least one issue, that's your queue. (Use `--search`, not `--label` — the parenthesised label name breaks the `--label` filter.) Each issue body contains an `## Acceptance criteria` section with `- [ ]` items.
...
- `bash tests/test-skills.sh` to validate skill format/content
- `bash tests/test-structure.sh` to validate repo layout
...
1. Start with `RALPH(converge-loop):` prefix
...
After committing, append a self-report to `docs/plans/converge-loop/agent-reports.log`. The overseer reads recent reports each tick and may emit steering directives based on what you flag. Be honest — concerns and uncertainties are more useful than reassurance.
