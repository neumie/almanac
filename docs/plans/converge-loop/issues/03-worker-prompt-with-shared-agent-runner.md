...
- [x] Each round spawns the worker via `agent_run` with `sandbox=workspace-write` (so it can commit). Provider resolved via `lib/role.sh::almanac_role_field "agent" "" provider` against `CONVERGE_AGENT_*` env (extend `lib/role.sh` if it currently hardcodes the consumer name; converge needs to be a recognized consumer alongside ralph and harden — keep the lookup table-driven, not branched).
