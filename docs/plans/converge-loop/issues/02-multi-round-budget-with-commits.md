...
- [x] Default round budget is `${CONVERGE_ROUND_BUDGET:-10}` (matches the harden / ralph convention of "env var or hardcoded default").
...
- [x] `almanac_loop_update_run_progress` is called at the end of each round with `round=<N>` and `summary=goal=<slug>` (or whatever the contract supports — match the field set ralph/harden write).
