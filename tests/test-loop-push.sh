#!/usr/bin/env bash
# test-loop-push.sh - Regression tests for Loop's auto-push behavior

set -euo pipefail

ROOT="$(cd "$(dirname "$0")/.." && pwd)"
source "$ROOT/skills/loop/loop/scripts/loop-git.sh"
...
test_mismatched_upstream_pushes_to_same_named_branch() {
  ...
  (
    ...
    git checkout -b loop-push-test origin/main >/dev/null 2>&1
    ...
    if ! loop_push_current_branch >/dev/null; then
      fail "loop push should repair mismatched upstream and push"
    fi

    assert_eq "origin/loop-push-test" "$(git rev-parse --abbrev-ref --symbolic-full-name "@{u}")" "loop push should set same-named upstream"
    assert_eq "$(git rev-parse HEAD)" "$(git --git-dir="$tmp/origin.git" rev-parse refs/heads/loop-push-test)" "remote branch should point at pushed commit"
  )
...
test_no_upstream_sets_same_named_branch() {
  ...
  (
    ...
    if ! loop_push_current_branch >/dev/null; then
      fail "loop push should push branches with no upstream"
    fi

    assert_eq "origin/no-upstream" "$(git rev-parse --abbrev-ref --symbolic-full-name "@{u}")" "loop push should set upstream for branch without one"
