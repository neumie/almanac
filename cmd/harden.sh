...
usage() {
  ...
  printf '%s\n' ""
  printf '%s\n' "<target> is what to harden: a path, a PR/branch ref (\"PR 47\"), or a free-form"
  printf '%s\n' "description. There is no filesystem check — reviewers locate the code themselves."
  printf '%s\n' ""
  ...
  printf '%s\n' "-> gate -> commit) in rounds until it converges or the round budget is hit."
  printf '%s\n' "Set the budget with --rounds N (default 5, or HARDEN_ROUND_BUDGET). Between"
  printf '%s\n' "rounds a HITL checkpoint lets you ship, continue, or steer — steering captures"
  printf '%s\n' "a directive that redirects the next round's reviewers and fixer. If the target"
  printf '%s\n' "is a git repo, each round commits its changes (fixer edits + tests + ledger,"
  printf '%s\n' "excluding .almanac/) as 'harden(<slug>): round N fixes (C)' with the addressed"
  printf '%s\n' "findings in the body; set HARDEN_AUTOCOMMIT=0 to leave fixes uncommitted. Start"
  printf '%s\n' "from a clean tree (the whole tree is staged)."
