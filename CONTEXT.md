...
- **new-run composition** — two verbs: `new_run_argv` (emit the hub's launcher argv, one token per line, from key=val pairs; returns 2 on a missing required field) and `new_run_env` (emit `KEY=VALUE` env lines for fields that ride on environment — empty for ralph by construction). Consumed by `almanac_loop_new_run_argv` / `_env` in `lib/run.sh`, both now thin dispatchers (no `case "$type"`). Latent bug it forecloses: the new-run composer growing a 4th branch (env prefixes, flag shapes) out of sync with the adapter — today there is no central branch to drift.
